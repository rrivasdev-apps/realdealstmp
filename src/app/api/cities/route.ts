...
// state_id is optional -- when omitted (Contact Hub's "Cities Interested In", which
// isn't state-scoped the way a deal address is), search falls back to a
// company-wide prefix search backed by cities_company_name_prefix_idx.
export async function GET(request: Request) {
  ...
  if (stateId) {
    builder = builder.eq('state_id', stateId)
  }
  if (query) {
