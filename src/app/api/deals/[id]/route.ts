...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const updatePayload = {
    ...
    title_company_id: body.title_company_id || null,
    mortgage_company_contact_id: body.mortgage_company_contact_id || null,
    mortgage_company_id: body.mortgage_company_id || null,
    ...
    jv_partner_contact_id: body.jv_partner_contact_id || null,
  ...
  // Runs after commission state has settled, so automations see fully-settled
  // numbers. "after" merges the update payload over the pre-update row so every
  // column has a real value to diff against, including ones this request didn't touch.
  await evaluateTriggersForDealUpdated(supabase, profile.company_id, id, existing, {
    ...existing,
    ...updatePayload,
  })
